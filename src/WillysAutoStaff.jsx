...
function PlaceholderPhoto() {
  return (
    <div className="aspect-square rounded-lg bg-brand-ink/5 border border-brand-ink/10 flex flex-col items-center justify-center text-brand-ink/65">
      <svg
        aria-hidden="true"
    ...
    <div className="min-h-screen bg-brand-cream text-brand-ink">
      ...
      >
        <div className="max-w-6xl mx-auto px-6 py-2 flex items-center justify-between">
          ...
          <div className="hidden md:flex gap-8">
            ...
            <a href={`${homeBase}staff`} className="font-medium text-brand-accent">Staff</a>
            <a href={`${homeBase}videos`} className="font-medium text-brand-cream hover:text-brand-accent transition-colors">See Our Work</a>
          </div>
          <button
            type="button"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-brand-surface border-t border-brand-accent/30 px-6 py-4">
            ...
            <a href={`${homeBase}staff`} className="block py-2 font-medium text-brand-accent">Staff</a>
            <a href={`${homeBase}videos`} className="block py-2 font-medium text-brand-cream hover:text-brand-accent transition-colors">See Our Work</a>
          </div>
        )}
      </nav>
      ...
      <section className="px-6 pt-28 pb-20">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold text-brand-ink mb-4 text-center">Meet Our Team</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {STAFF.map((member, idx) => (
              <article
                key={idx}
                className="rounded-xl border border-brand-ink/10 bg-white p-5 md:p-6 shadow-sm flex flex-col"
              >
                <PlaceholderPhoto />
                <div className="mt-5">
                  <h3 className="text-xl font-bold text-brand-ink">{member.name}</h3>
                  <p className="text-sm font-semibold text-brand-accent mt-1">{member.title}</p>
                  <p className="text-brand-ink/80 mt-3 leading-relaxed">{member.bio}</p>
                </div>
              </article>
            ))}
          </div>
